...
Style stdlib `unittest` (cf. README : `python -m unittest discover -s tests`).
"""
import os
import sys
import unittest
...
class ComputeQueuesTest(unittest.TestCase):
    def test_triage_si_pas_de_demande_ou_pas_de_person(self):
        sols = [
        ...
        self.assertEqual({s["id"] for s in q["triage"]}, {"a", "b"})

    def test_accuse_seulement_recent_et_demande_active_sans_accuse(self):
        sols = [
        ...
        q = recontact.compute_queues(sols, ack_max_age_days=3)
        self.assertEqual({s["id"] for s in q["accuse"]}, {"frais"})

    def test_merci_si_livree_et_pas_encore_remercie(self):
        sols = [
        ...
        q = recontact.compute_queues(sols, ack_max_age_days=3)
        self.assertEqual({s["id"] for s in q["merci"]}, {"a"})

    def test_livree_recente_sans_accuse_va_en_merci_pas_en_accuse(self):
        ...
        q = recontact.compute_queues(sols, ack_max_age_days=3)
        self.assertEqual({s["id"] for s in q["merci"]}, {"x"})
        self.assertEqual(q["accuse"], [])
